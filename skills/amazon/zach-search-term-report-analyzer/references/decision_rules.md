...
当 ASIN 所在类目为小类目（搜索量主导词只有 1-2 个，长尾池稀薄）时，**不要对核心大词直接建议降 bid / 降权 / 否定**，即使它 ACOS 高达 100%+。理由：

- 小类目没有足够长尾池替代核心词的流量
- 核心词广告位丢失后自然位会在 1-2 周内跟着塌
- 核心词 ACOS 虚高的根因通常是 **CTR 过低（主图没抢到点击）** 和 **CVR 过低（Listing / Review 不够）**，不是出价问题

识别"小类目 + 核心词"信号：

- 该词月搜索量 / 类目 Top1 月销量比值异常高
- 用户明确声明"不能否这个词"或"这是我的核心词"
- 同 ASIN 的其他搜索词总点击量远小于该核心词

对这类词的正确输出：

- 主标签：`core_category_term`
- 动作标签：`hold_test`（不要给 `reduce_bid` 或 `negative_candidate`）
- 必须在说明里写清 **"效率提升要靠 listing 创意和广告位优化，不应降 bid"**，并推荐：
  1. 广告位溢价重调：Top of Search +30~100%，Product Pages 归零或负溢价
  2. 竞价策略改为"仅降低"（Down only）
  3. 单独拆一个 Top-of-Search-only campaign 守位
  4. 主图改版（CTR 杠杆）— 具体要求见 `amazon-listing-creative` skill
  5. 副图 + bullet + Vine（CVR 杠杆）— 具体要求见 `amazon-listing-creative` skill

只有在核心词是**无关宽词**（如做 `trail camera` 的卖家跑出 `camera` 这种宽词）时才可以否。识别判据：
